...
class GenerateTFRecord:
    def __init__(self, outpath, num_trecords, filesize, unlvimagespath, unlvocrpath, unlvtablepath, visualizeimgs, visualizebboxes, distributionfilepath):
        self.outtfpath = outpath  # directory to store tfrecords
        self.num_trecords = num_trecords
            ...
            for _ in range(cat_count):
                ...
                while (True):
                    ...
                    try:
                        ...
                        yield [same_row_matrix, same_col_matrix, same_cell_matrix, bboxes, [tablecategory]], [im]
                ...
                rc_count += 1
                if rc_count % 100 == 0:
                    print(f'Generate {rc_count} samples')
    ...
    def write_tf(self, is_stop, increase, filesize, threadnum):
        ...
        while not is_stop():
            ...
            with tf.io.TFRecordWriter(os.path.join(self.outtfpath, output_file_name), options=options) as writer:
                try:
                    ...
                    print('\nThread :', threadnum, ' Completed in ', time.time() - starttime, ' ', output_file_name, 'with len:', (num_gen))
    ...
    def write_to_tf(self, max_threads):
        ...
        self.total_sample = 0
        is_stop = lambda: self.total_sample > self.num_trecords
        ...
        for threadnum in range(max_threads):
            proc = Process(target=self.write_tf, args=(is_stop, _increase, self.filesize, threadnum,))
            proc.start()
            threads.append(proc)
